...
CREATE TABLE `ad` (
	...
	`ad_text` text,
	`status` ENUM('active', 'saled', 'remove'),
	`created_at` DATETIME DEFAULT NOW(),
...
CREATE TABLE `deal` (
	...
	`complited_at` datetime DEFAULT now(),
	
	FOREIGN KEY (ad_id) REFERENCES ad(id) ON UPDATE CASCADE ON DELETE CASCADE,
	FOREIGN KEY (seller_id) REFERENCES ad(user_id) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE `user_deals` (
	...
	FOREIGN KEY (user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE,
	FOREIGN KEY (deal_id) REFERENCES deal(id) ON UPDATE CASCADE ON DELETE CASCADE
	);
